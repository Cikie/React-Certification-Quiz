  ...
  async ({ categoryId, difficulty }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        "There is error when get list questions! Please try again!"
      );
    }
  }
);
...
const questionSlide = createSlice({
  ...
  reducers: {
    chooseAnswerAction: (state, { payload }) => {
      ...
      const oldAnswerIndex = state.choosedAnswers.findIndex(
...
export const { chooseAnswerAction, resetQuestionAction } =
